...
public:
	...
	void handle_get_request(RequestObjectUPtr transport_object)
	{
		...
		if(succ == nullptr)
		{
			auto find_succ_req_msg = std::make_shared<Message>(Message::create_request("get"));
		...
		auto get_internal_message = std::make_shared<Message>(Message::create_request("get_internal"));
	...
	void handle_put_request(RequestObjectUPtr transport_object)
	{
		...
		if(succ == nullptr)
		{
			...
			auto find_succ_msg =
				std::make_shared<Message>(Message::create_request("find_successor", {Peer(data_id, "", "")}));
			...
			auto store_msg = std::make_shared<Message>(Message::create_request("store"));
	...
	void handle_find_successor_request(RequestObjectUPtr transport_object)
	{
		...
		if(successor == nullptr)
		{
			...
			auto new_request =
				std::make_shared<Message>(Message::create_request("find_successor", {message->get_peers().front()}));
	...
	void stabilize_storage()
	{
		...
		for(const auto &file : files)
		{
			...
			if(util::between(predecessor.get_id(), file.first, self.get_id()))
			{
				...
				auto backup_msg = std::make_shared<Message>(Message::create_request("backup"));
				backup_msg->set_data(file.first);
				auto transaction_id = backup_msg->get_transaction_id();
				auto handler = std::bind(&MessageHandler::handle_backup_response, this, std::placeholders::_1);
	...
	void handle_backup_response(RequestObjectUPtr transport_object)
	{
		...
		store_request->set_message(store_msg);
		store_request->set_connection(std::make_shared<Peer>(transport_object->get_message()->get_peers().front()));
		push_to_write_queue(store_request);
	}

	std::future<std::string> get(const std::string &ip, const std::string &port, const std::string &data)
	{
		auto get_request_message = std::make_shared<Message>(Message::create_request("get"));
		...
		// maybe add a possibility to create response handler functions that also
		// have access to the original request? right now a normal response handler
		// has only access to the response object, not the original request that was
		// sendet (and that is holding a promise value)
		auto dummy_message = std::make_shared<Message>(Message::create_request("get_dummy"));
	...
	std::future<std::string> put(const std::string &ip, const std::string &port, const std::string &data)
	{
		// generate data id
		auto data_id = util::generate_sha256(data, "");
		...
		auto put_request_message = std::make_shared<Message>(Message::create_request("put"));
		...
		auto dummy_message = std::make_shared<Message>(Message::create_request("put_dummy"));
