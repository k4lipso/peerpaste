...
BroadcastFilelist::BroadcastFilelist(ConcurrentRoutingTable<Peer>* routing_table, StaticStorage* storage, RequestObject request)
...
{
	if(std::all_of(dependencies_.begin(), dependencies_.end(),
								 [](auto dep){ return dep.first->is_done(); }))
	{
...
void BroadcastFilelist::create_request()
{
  ...
  const auto message =
      std::make_shared<Message>(Message::create_request("broadcast_filelist", {self}));
...
void BroadcastFilelist::handle_request()
{
  ...
  const auto new_message =
      std::make_shared<Message>(Message::create_request("broadcast_filelist", peers));
