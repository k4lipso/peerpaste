...
void Query::create_request()
{
	...
	// Generate Query
	const auto query_request = std::make_shared<Message>(Message::create_request("query", {self}));

	const auto transaction_id = query_request->get_transaction_id();

	const auto target = std::make_shared<Peer>(target_);

	const auto handler = std::bind(&Query::handle_response, this, std::placeholders::_1);
